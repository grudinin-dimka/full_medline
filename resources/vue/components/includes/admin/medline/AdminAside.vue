<template>
	<VueAside :is-hide="$store.getters.getIsHide">
		...
		<template #body>
			...
			<VueAsideList :alias="['statistics-list', 'statistics-graph']">
				<template #title>
					<VueIcon :name="'Monitoring'" :fill="'black'" :width="'28px'" :height="'28px'" />
					<span v-if="!$store.getters.getIsHide" class="aside__item-button-label"> СТАТИСТИКА </span>
				</template>
				...
				<template #content>
					...
					>
						<VueIcon :name="'Monitoring'" :fill="'black'" :width="'28px'" :height="'28px'" />
						<span v-if="!$store.getters.getIsHide" class="aside__item-button-label"> ГРАФИКИ </span>
					</VueAsideButton>
				</template>
			</VueAsideList>
