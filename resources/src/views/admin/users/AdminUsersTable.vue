	...
	<table>
		<thead>
			<tr>
				...
				<th>
					<div class="table-th-container">
						<div class="table-th-name">ФИО</div>
						<div
							class="table-th-filter"
				...
				<th>
					<div class="table-th-container">
						<div class="table-th-name">Статус</div>
						<div
							class="table-th-filter"
							v-if="!filters.specialization.status"
							@click="useFilter('status', 'default')"
						>
							<icon-filter-off :width="22" :height="22" />
						</div>
						<div
							...
							"
							@click="useFilter('status', 'reverse')"
						>
							<icon-filter-on-reverse :width="22" :height="22" />
						</div>
						<div
				...
				<th>
					<div class="table-th-container">
						<div class="table-th-name">Права</div>
						<div
							class="table-th-filter"
							v-if="!filters.hide.status"
							@click="useFilter('rights', 'default')"
						>
							<icon-filter-off :width="22" :height="22" />
						</div>
						<div
							...
							@click="useFilter('rights', 'reverse')"
						>
							<icon-filter-on-reverse :width="22" :height="22" />
						</div>
						<div
		...
		<tbody>
			...
			<tr
				...
				:class="{ create: element.create, delete: element.delete }"
			>
				<td>
					{{ element.id }}
				</td>
				<td>
					<div class="info">
						<div class="fio">
							{{ element?.family + " " + element?.name + " " + element?.surname }}
						</div>
						<div class="nickname">
							{{ element?.nickname }}
						</div>
				...
				<td>
					<table-container-buttons>
						<table-button-default
							v-if="!element.delete"
							@click="$emit('touchRemoveElement', element)"
						>
							Открыть
						</table-button-default>
						<TableButtonDisabled v-if="element.delete"> Открыть </TableButtonDisabled>

						<table-button-remove
							@click="$emit('touchRemoveElement', element.id)"
							v-if="!element.delete"
						>
							Удалить
						</table-button-remove>
